...
if GEMINI_API_KEY:
    try:
        ...
        print("[ADVISOR] Conectado a Gemini 2.5 Flash (SDK google-genai).")
    except Exception as e:
        print(f"[ADVISOR] Error configurando google-genai: {e}")
else:
    print("[ADVISOR] GEMINI_API_KEY no encontrada. Advisor desactivado.")

MODEL_ID = "gemini-2.0-flash"

async def generate_tactical_advice(asset: str, tactical_data: dict, current_session: str, ml_projection: dict = None) -> str:
    """
    Genera un consejo cuantitativo breve usando Gemini LLM (SDK google-genai) de forma asíncrona.
    """
    if not client:
    ...
    in_killzone = "SI (Volumen Institucional Alto)" if diag.get('in_killzone', False) else "NO (Volumen Minorista/Lento)"
    ...
    sigs_desc = ""
    if active_signals:
        sigs_desc = "SENALES ACTIVAS GENERADAS POR EL ROUTER:\n" + "\n".join([
            f"- {s.get('type', 'UNKNOWN')} @ ${f_p(s.get('price', s.get('entry_price', 0)))} (Conf: {s.get('confluence', {}).get('score', 0)}%)" 
            for s in active_signals
        ])
    else:
        sigs_desc = "NO SE HAN GENERADO SENALES NUEVAS EN ESTE TICK (Esperando confluencia)."

    inv_sigs_desc = ""
    if inv_signals:
        inv_sigs_desc = "SENALES RECIENTEMENTE INVALIDADAS/MUERTAS:\n" + "\n".join([
    ...
    Tu mision es explicar la logica del motor tecnico, NO contradecirla ni inventar datos.
    
    DATOS DEL MOTOR EN TIEMPO REAL ({asset}):
    - Regimen Wyckoff (LA VERDAD ABSOLUTA): {regime}
    - Estrategia del Router: {strategy} | KillZone: {in_killzone}
    - Proyeccion IA XGBoost (PROHIBIDO ALTERAR): {ml_dir} ({ml_prob}%)
    - RSI: {rsi:.1f} | MACD: {macd_cross} | BBWP: {bbwp:.1f}%
    
    ESTADO DE SENALES (GROUND TRUTH):
    {sigs_desc}
    {inv_sigs_desc}
    ...
    1. PRIORIDAD DE REGIMEN: Si el Regimen es MARKDOWN, tu sesgo DEBE ser bajista. Si es MARKUP, alcista.
    2. ZERO HALLUCINATION: No inventes porcentajes. Si el motor dice {ml_prob}%, usa ese numero exacto.
    3. TONO: Militar, gelido, tecnico, MAYUSCULAS.
    4. MAXIMO 3 ORACIONES. No cortes la frase.
    5. Solo recomienda accion si el motor genero SENALES ACTIVAS. Si no hay senales, recomienda ESPERAR.
    6. AL FINAL de tu mensaje, DEBES incluir obligatoriamente el Ratio Riesgo/Beneficio en el formato [R:R TGT {recommended_rr}]

    Ejemplo de respuesta ideal:
    MAXIMA CONFLUENCIA ALCISTA DETECTADA EN LONDRES KILLZONE. PRECIO APOYADO SOBRE DEMANDA INSTITUCIONAL CON IA PROYECTANDO 62% DE PROBABILIDAD. DESPLEGAR ORDENES LARGAS APUNTANDO A LA RESISTENCIA MAS CERCANA. [R:R TGT 1:3]
    """
    ...
    try:
        ...
        print(f"[ADVISOR] Analisis generado para {asset} ({len(advice)} chars)")
        return advice
    except Exception as e:
