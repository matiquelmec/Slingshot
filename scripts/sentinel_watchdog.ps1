    ...
    try {
        ...
        if ($sidecarFails -ge 2) {
            ...
            if (Test-Path $sidecarScript) {
                Start-Process node -ArgumentList "`"$sidecarScript`"" -WorkingDirectory $root -WindowStyle Hidden
        ...
        try {
            $apiRes = Invoke-RestMethod -Uri "http://127.0.0.1:8000/api/v1/health" -Method Get -TimeoutSec 5 -ErrorAction Stop
        ...
        if ($fastapiFails -ge 2) {
            ...
            Start-Process $pyExe -ArgumentList "-m uvicorn engine.api.main:app --host 0.0.0.0 --port 8000" -WorkingDirectory $root -WindowStyle Hidden
