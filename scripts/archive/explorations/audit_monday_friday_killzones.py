...
async def run_monday_friday_audit():
    print("=" * 95)
    print("🎯 AUDITORÍA ESPECÍFICA: LUNES + VIERNES EN APERTURAS DE LONDRES & NUEVA YORK (6 MESES)")
    print("=" * 95)

    router = SlingshotRouter()
    
    scenarios = {
        "1. TODOS LOS DÍAS & HORARIOS (>50% + EMA 200)": [],
        "2. SOLO LUNES Y VIERNES (Cualquier Hora)": [],
        "3. SOLO APERTURA LONDRES Y NY (Cualquier Día)": [],
        "4. FILTRO ESTRICTO: SOLO LUNES Y VIERNES EN LONDRES/NY OPEN": []
    }
    ...
    for file_name in FILES:
        ...
        for i in range(window_size, len(df) - 96, 24):
            ...
            is_mon_fri = (day_name in ["Monday", "Friday"])
            is_london_ny_open = (7 <= hour_utc < 10) or (12 <= hour_utc < 16)
            ...
            for direction in ["LONG", "SHORT"]:
                ...
                if score >= 50 and is_trend_ok:
                    ...
                    if sl > 0 and tp1 > 0:
                        ...
                        scenarios["1. TODOS LOS DÍAS & HORARIOS (>50% + EMA 200)"].append(pnl)

                        if is_mon_fri:
                            scenarios["2. SOLO LUNES Y VIERNES (Cualquier Hora)"].append(pnl)

                        if is_london_ny_open:
                            scenarios["3. SOLO APERTURA LONDRES Y NY (Cualquier Día)"].append(pnl)

                        if is_mon_fri and is_london_ny_open:
                            scenarios["4. FILTRO ESTRICTO: SOLO LUNES Y VIERNES EN LONDRES/NY OPEN"].append(pnl)

    print("\n" + "=" * 95)
    print("📊 RESULTADOS MATEMÁTICOS DE LA ESTRATEGIA SOLICITADA (6 MESES)")
    print("=" * 95)
...
if __name__ == "__main__":
    asyncio.run(run_monday_friday_audit())
