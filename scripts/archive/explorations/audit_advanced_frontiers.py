...
def simulate_trade_smc_trailing(df, start_idx, direction, entry, sl, tp1, tp2, tp3, max_holding_bars=96):
    ...
    for i in range(start_idx, end_idx):
        ...
        if direction == "LONG":
            # Trailing stop por estructura: Si alcanza TP1, mover SL a Breakeven (entry)
            if hit_tp1 and current_sl < entry:
                current_sl = entry
            # Si alcanza TP2, mover SL a TP1 (asegurar profit)
            if hit_tp2 and current_sl < tp1:
                current_sl = tp1

            if low <= current_sl:
                if current_sl == tp1:
                    return "TP1_TRAIL", 100.0
...
async def audit_advanced_frontiers():
    print("=" * 95, flush=True)
    print("🔬 AUDITORÍA DE FRONTERAS AVANZADAS: TRAILING SMC & ALINEACIÓN MULTI-TIMEFRAME", flush=True)
    print("=" * 95, flush=True)
    ...
    results = {
        "0. BASE V11.3 (KER Adaptativo)": [],
        "F1. TRAILING STOP ESTRUCTURAL SMC (Breakeven en TP1, Proteger en TP2)": [],
        "F2. FILTRO DE CONFLUENCIA ALTA (Score >= 60%)": [],
        "F3. COMBO SOVEREIGN (Veto BTC + Trailing SMC + Score >= 60%)": []
    }
                ...
                if sl > 0 and tp1 > 0 and score >= 50 and is_trend_ok and is_htf_aligned:
                    ...
                    if btc_df is not None and symbol != "BTCUSDT":
                        ...
                        if len(btc_sub) > 0:
                            ...
                            btc_aligned = (direction == "LONG" and b_price > b_ema200) or (direction == "SHORT" and b_price < b_ema200)

                    # 0. Base static
                    from scratch.audit_unexplored_frontiers import simulate_trade_adaptive_tp
                    _, base_pnl = simulate_trade_adaptive_tp(df, i, direction, optimal_entry, sl, tp1, tp2, tp3)
                    results["0. BASE V11.3 (KER Adaptativo)"].append(base_pnl)

                    # F1. Trailing SMC
                    _, trail_pnl = simulate_trade_smc_trailing(df, i, direction, optimal_entry, sl, tp1, tp2, tp3)
                    results["F1. TRAILING STOP ESTRUCTURAL SMC (Breakeven en TP1, Proteger en TP2)"].append(trail_pnl)

                    # F2. Confluencia Alta >= 60%
                    if score >= 60:
                        results["F2. FILTRO DE CONFLUENCIA ALTA (Score >= 60%)"].append(base_pnl)

                    # F3. COMBO SOVEREIGN
                    if score >= 60 and btc_aligned:
                        results["F3. COMBO SOVEREIGN (Veto BTC + Trailing SMC + Score >= 60%)"].append(trail_pnl)

    print("\n" + "=" * 95, flush=True)
    print("📊 RESULTADOS COMPARATIVOS DE LAS FRONTERAS AVANZADAS", flush=True)
    print("=" * 95, flush=True)

    for h_name, pnls in results.items():
        total_t = len(pnls)
        ...
        print(f"   • Ganadoras / Perdedoras / BE: {wins} / {losses} / {be}", flush=True)
...
if __name__ == "__main__":
    asyncio.run(audit_advanced_frontiers())
